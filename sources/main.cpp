...
void    printArrayInt(string text, const int *array, int size) {
    cerr << text << " : ";
...
void    printArrayCyl(string text, const t_cylinder *array, int size) {
    cerr << text << " : ";
    for (int i = 0; i < size; i++) {
        cerr << "(" << array[i].position << ", " << array[i].radius << "), ";
    }
    cerr << endl;
}
...
//cerr << "inside next_order" << endl;
    while (position >= 0) {
        ...
        while (++candidate < size && (isUsed(candidate, order, position) || sequence[candidate] == sequence[curCandidate]
            ...
            || (candidate > size / 2 + size % 2 && position < size -  1 && (order[position + 1] == candidate - 1 || order[position - 1] == candidate - 1))))
                ;
        if (candidate > size - 1)
            position--;
        else {
    ...
    //printArrayInt("Next order : ", order, size);
    return (0);
}
...
double min_distance(int *original_sequence, int size) {
    ...
    //printArrayInt("original_sequence", original_sequence, size);
    ...
    while (!next_order(original_sequence, order, size)) {
        copy_array(original_sequence, size, sequence);
        order_sequence(sequence, size, order);
        //printArrayInt("current_sequence", sequence, size);
        cur_dist = compute_distance(sequence, size);
        //if ((int)cur_dist == 3775)
        //        printArrayInt("Order min", order, size);
...
{ 
    ...
    for (int i = 0; i < n; i++) {
        ...
        //cerr << "size numbers : " << nb_elem << endl;
        //printArrayInt("input", num, nb_elem);
        if (nb_elem == 1)
            distance = 2.0 * num[0];
        else
